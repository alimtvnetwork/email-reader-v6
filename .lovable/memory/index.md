...
- [Settings redesign Phase 1 (Slice #212)](mem://workflow/settings-redesign-p1-slice212.md) — 9 flat rows → 3 cards (Appearance/Watcher/Maintenance); paths-card with Copy/Open/Reveal; `browser.RevealInFileManager` keeps OI-5 single-allowlist intact; AC-SF-03 audit re-pinned. P2–P6 of UX roadmap (sidebar, error-log per-row actions, dashboard, accounts, theme polish) queued — execute on "next".
- [Sidebar hit-area & visual polish (Slice #213)](mem://workflow/sidebar-hit-area-slice213.md) — Padded row template (~24-30px hit target) + active-row "▸ " caret + 2-space header inset; new pure `SidebarRowText` helper headless-tested. Fixes the "Diagnose opens when I click Error Log" misclick complaint.
