...
Last updated: 2026-04-26 (UTC) — Slice #33 landed: structured-logging migration for the maintenance loop. `internal/ui/maintenance_log.go` now uses `log/slog` with a lazy package-private logger (`maintenanceSlog`) carrying `component=maintenance` via `slog.With`; emits INFO on success and WARN on error per spec/23-app-database/04 §6. The four `Format*` helpers were re-canonicalised to return the spec event tail (`event=prune deleted=N ok|error=…`, `event=analyze triggered_at=N …`, `event=vacuum reclaimed_bytes=N …`, `event=wal_checkpoint pages=N …`) instead of the old `ui: maintenance: …` human prefix; production callbacks (`logRetentionSweep`/`logAnalyzeRun`/`logVacuumRun`/`logWalCheckpoint`) route through a new `emitMaintenance(event, key, val, err)` shared helper. Test suite refactored: existing format-pinning tests in `maintenance_log_test.go` + `maintenance_log_jobs_test.go` updated to the new tail; new `maintenance_log_emit_test.go` swaps the slog sink to a `bytes.Buffer` and verifies `component=maintenance` + the right `event=…` key + `level=INFO|WARN` are emitted by every callback (5 cases). All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -race -count=2 ./...`; `go vet -tags nofyne ./...` clean.
