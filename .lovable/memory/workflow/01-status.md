...
Last updated: 2026-04-26 (UTC) — **Slice #42 landed: Density preference persistence end-to-end.** **New error code:** `ErrSettingsDensity = ER-SET-21783`. **New file:** `internal/core/settings_density_test.go` (6 tests: ParseDensity, String, Default, full Save+reload round-trip with disk re-read, lenient zero-value normalize, validator rejects junk). **Files edited:** `internal/core/settings_types.go` — new `Density` enum (Comfortable=1, Compact=2 — non-zero so the zero value can mean "use default"), `String()`, `ParseDensity`, added `Density` field to `SettingsInput`/`SettingsSnapshot`/`DefaultSettingsInput`. `internal/core/settings.go` — `density` field on `settingsExtension` JSON, plumbed through `applyInputToRaw`, `projectExtension` (with empty-string → default fallback), and `snapshotFromRaw`. `internal/core/settings_validate.go` — `validateDensity` + `normalizeInput` substitutes default for zero (mirrors WalCheckpointHours pattern). `internal/ui/app.go` — `loadInitialDensity()` mirrors `loadInitialThemeMode()`; `theme.SetDensity(loadInitialDensity())` runs in `Run()` BEFORE `BuildShell` so the very first paint already uses persisted spacing (no Comfortable→Compact pop on second frame); `forwardThemeEvents` extended to also re-apply density on every Settings event with its own sentinel-zero `lastDensity` cache; `coreDensityToTheme` bridge translates the offset (core 1/2 → theme 0/1). `internal/ui/views/settings.go` — Density Select pre-fills from `snap.Density` (was `theme.ActiveDensity()`); Save now persists via the new `densityLabel` arg to `ProjectSettingsInput`; `repopulateWidgets` restores density on Reset; hint text updated to "Persists across restarts." `internal/ui/views/settings_logic.go` — `DensityLabelFor`/`ParseDensityChoice` switched to `core.Density` int form; new pure `CoreDensityToThemeDensity` bridge (testable under nofyne); `ProjectSettingsInput` gains trailing `densityLabel string` arg. `internal/ui/views/settings_logic_test.go` — updated round-trip test for new ints + 4 ProjectSettingsInput call sites + new `Test_CoreDensityToThemeDensity` to lock the offset. **Verified:** all 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -count=1 ./...`; `go vet -tags nofyne ./...` clean.

## Previous slice (kept for context)
Slice #41 landed: app boot smoke test in `internal/core/boot_smoke_test.go` — 2 tests walking `NewSettings → Get → Subscribe → ListAccounts → ListRules` on a fresh data dir + repeated-construction leak guard, with a 2s cold-start budget. Tag-free so it runs under the project's standard `-tags nofyne` verification.
