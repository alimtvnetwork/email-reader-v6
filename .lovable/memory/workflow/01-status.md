...
Last updated: 2026-04-26 (UTC) — Slice #36 landed: AST guard test for AC-DB-51 (`Test_AST_NoSqlTypeLeak`). New `internal/store/ast_no_sql_type_leak_test.go` walks `internal/store/` only (sub-packages included via WalkDir), parses every production `.go` file (tests exempt), and for each top-level `*ast.FuncDecl` whose name is exported (and whose receiver is itself exported, so unreachable methods on unexported receivers don't trip the guard) inspects the return list with `ast.Inspect`. Any `*ast.SelectorExpr` of the form `sql.DB`/`sql.Tx`/`sql.Rows` at any depth (bare pointer, slice element, map value, channel element, generic type-arg) is reported as `<rel>:<line> Receiver.Method: returns sql.X`. Function-length compliance via 7 small helpers (`scanStoreForSqlReturnLeaks`, `inspectStoreGoFile`, `scanFileForSqlReturnLeaks`, `isExportedFuncDecl`, `leaksInReturnList`, `formatLeakHit`, `receiverTypeName`, `unwrapPointerType`, plus a stdlib-free `itoa` to keep the import block lean). Today no public store method returns a `sql.*` type — guard passes clean. **Architectural gap noted (out of scope for AC-DB-51's parenthetical):** `Store.DB *sql.DB` is exported and read directly by `internal/exporter`, `internal/core/tools_export.go`, `internal/core/tools_diagnose.go`. Closing that leak is AC-DB-52's territory and will need a typed-method shim. All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -race -count=2 ./...`; `go vet -tags nofyne ./...` clean; fn-length linter 0 violations on the new file.
