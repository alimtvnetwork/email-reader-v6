...
| 26 | **Retention sweep observability (this slice)** — wired the previously-unused `MaintenanceOptions.OnSweep` seam to a stdlib-`log` callback. New `internal/ui/maintenance_log.go` exports a pure `FormatRetentionSweep(deleted int64, err error) string` (the formatter) and an unexported `logRetentionSweep` that calls `log.Print` (the production sink). `startMaintenance` in `watch_runtime.go` now passes `OnSweep: logRetentionSweep`, so every retention tick — success OR error, including the common zero-deleted heartbeat — emits a single greppable line: `"ui: maintenance: retention sweep: deleted=N ok"` or `"ui: maintenance: retention sweep: deleted=N error=<msg>"`. The format is locked by `internal/ui/maintenance_log_test.go` (4 cases: success-with-count, zero-count-still-logged, error-preserves-count-and-msg, single-line-in-both-branches). Splitting formatter from sink keeps the line shape testable without a logger fixture and prepares a clean cut-over to slog when the rest of the codebase migrates. Verification: `go vet ./...` (both tag matrices) clean; `go build -tags nofyne ./...` clean; `-race ./internal/{ui,core}` green; fn-length linter clean across edited files. | `internal/ui/maintenance_log.go`, `internal/ui/maintenance_log_test.go`, `internal/ui/watch_runtime.go` |

Verification: 17 packages green under `nix run nixpkgs#go -- {vet,test} -tags nofyne ./...`; `go build -tags nofyne ./...` clean; race sweep `-race ./...` green; fn-length linter **0/0** across edited Go files.

## Remaining tracked work

See `spec/21-app/99-consistency-report.md` §6 for the canonical delta list. Open items:

1. **App boot smoke test** (user-side) — launch desktop binary; validate Settings render/live-switch, density toggle, Watch Start/Stop, Dashboard tiles incrementing, Recent opens against a populated DB, retention-days field round-trips a Save, **and at least one `ui: maintenance: retention sweep` log line appears within ~1min of boot**. (Requires manual user run.)
...
6. **VACUUM / ANALYZE / WAL checkpoint** — spec/23-app-database/04 §2 documents the full maintenance schedule; we only ship the prune part. ANALYZE-after-1k-deletes and weekly-VACUUM are still TODO.
7. **Frontend `CODE-RED-004` cleanup** — `src/pages/NotFound.tsx` (16 lines) needs extraction into a smaller helper. Trivial.

## Next logical step for the next AI session
Top candidates (sandbox-runnable): (a) **#3 Watch backoff jitter** — next greenfield feature slice (exponential backoff + jitter on consecutive `EventPollError` + `CF-W-BACKOFF` test); (b) **#6 ANALYZE-after-N-deletes** — extend `internal/store/vacuum.go` to ANALYZE after every 1k cumulative deletes (weekly VACUUM can follow); (c) **#7 NotFound.tsx fn-length cleanup** — 5-minute lint hygiene win. Items #1/#2/#4/#5 still need user-side or CI infra.
