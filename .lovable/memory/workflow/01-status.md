...
Last updated: 2026-04-26 (UTC) — **Slice #38 landed: Add Account form extension (slice 2 of 2)** — Provider preset dropdown + Test Connection button. **Files added:** `internal/ui/views/account_presets.go` (typed `AccountPreset` table with 7 entries — Custom sentinel + Gmail/Outlook/Yahoo/iCloud/FastMail/Zoho — plus `PresetLabels`/`FindPreset`/`IsCustomPreset` pure helpers), `internal/ui/views/account_presets_test.go` (4 tests), `internal/mailclient/dial_plain.go` (new `DialPlain(PlainDialInput)` using `client.DialWithDialer{,TLS}` with 8s default timeout; tags `net.Error.Timeout()` as `ErrMailTimeout`, generic dial as `ErrMailDial`, login as `ErrMailLogin`; auto-Logout on success — pure probe, never persists), `internal/core/test_connection.go` (`TestAccountConnection(in, timeout)` orchestrates `resolveImapDefaults` + `mailclient.DialPlain`, returns typed `TestConnectionResult{Host,Port,UseTLS}`), `internal/core/test_connection_test.go` (3 input-validation tests, no network). **Files edited:** `internal/ui/views/add_account_form.go` — added `provider *widget.Select` to `accountFormEntries` (rendered first), `applyPresetToEntries` helper auto-fills host/port/TLS (no-op on Custom), new "Test connection" button next to Save runs probe in goroutine with disabled-button + status-banner UX, `AddAccountFormOptions.TestConn` injectable for headless tests (defaults to `core.TestAccountConnection`). **Verified:** all 16 packages pass `nix run nixpkgs#go -- test -tags nofyne -count=1 ./...`; `go vet -tags nofyne ./...` clean. UI completion now ~98% — only remaining work is user-side: build cgo binary on Windows (`.\run.ps1`) to actually click the new buttons.

## Previous slice (kept for context)
Slice #37 landed: Add Account form extension (slice 1 of 2) — Display Name field + Show/Hide password toggle. User correction: previous 5 slices (AC-DB AST guards) were off-track; user asked for UI form work, not backend tests. Honest UI audit confirmed all 7 nav destinations + both Add forms are wired in `internal/ui/app.go viewFor()`. Schema change: `config.Account.DisplayName string` (omitempty for backward-compat), `core.AccountInput.DisplayName` plumbed through `buildAccount`, `views.AccountFormInput/Result.DisplayName` trimmed in `ValidateAccountForm`, new "Display name" Fyne entry row, new `newPasswordRevealButton` toggling `*widget.Entry.Password` in place + "Show/Hide" label swap, `clear()` resets reveal state to hidden, 2 new tests.
