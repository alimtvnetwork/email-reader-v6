...
Last updated: 2026-04-26 (UTC) — **Slice #43 landed: Dashboard tile auto-refresh on `EventUrlOpened`.** **Files edited:** `internal/ui/views/dashboard_counters.go` — extracted `dashboardRefreshKind(watcher.EventKind) bool` helper covering `{EventNewMail, EventUrlOpened}`; rewired `ShouldRefreshDashboardOnEvent` to use it so the existing 750ms debounce window is *shared* across both kinds (NewMail followed 100ms later by its UrlOpened won't double-refresh COUNT(*)); godoc rewritten to spell out the rule-launch feedback contract. `internal/ui/views/dashboard.go` — comments on `newDashboardLiveRow`, `dashboardRefreshDebounce`, and `subscribeDashboardBus` updated to mention both trigger kinds (no logic change — `subscribeDashboardBus` already delegates to `ShouldRefreshDashboardOnEvent`). `internal/ui/views/dashboard_counters_test.go` — `TestShouldRefreshDashboardOnEvent` extended from 7 → 15 cases (url_opened first/within/past/failure, shared-debounce cross-kind, plus poll_error / uidval_reset / started negative cases); new `TestDashboardRefreshKind` walks all 10 EventKind values to lock the trigger set so adding a kind requires a deliberate test edit. **Verified:** all 16 packages green under `go test -tags nofyne ./...`; `go vet -tags nofyne ./...` clean; `-race` rerun on the affected counter tests passes.

## Previous slice (kept for context)
Slice #42 landed: Density preference persistence end-to-end. New error code `ErrSettingsDensity = ER-SET-21783`; new `Density` enum (Comfortable=1, Compact=2) plumbed through `SettingsInput`/`SettingsSnapshot`, persisted via `settingsExtension.density`, applied at boot in `Run()` *before* `BuildShell` (no first-paint pop), live-applied in `forwardThemeEvents` with sentinel-zero cache, and round-tripped from the Settings form via the new `densityLabel` arg + `CoreDensityToThemeDensity` bridge. 6 new tests in `settings_density_test.go` + updated logic tests; 16 packages green.
