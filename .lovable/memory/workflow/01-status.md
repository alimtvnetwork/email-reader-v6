...
Last updated: 2026-04-26 (UTC) — **Slice #40 landed: Edit Rule form + per-row Edit/Delete buttons in Rules view.** **New files:** `internal/ui/views/rule_form_labels.go` (tag-free `ruleSubmitLabel`/`ruleClearLabel` so labels are testable under `nofyne`); `internal/ui/views/add_rule_form_edit_test.go` (3 tests: submit/clear label flips + edit-mode round-trip through `ValidateRuleForm`). **Files edited:** `internal/ui/views/add_rule_form.go` — added `Initial *config.Rule` to `AddRuleFormOptions` (nil ⇒ Add, non-nil ⇒ Edit). Save still defaults to `core.AddRule` because that already does upsert-by-name; no separate `UpdateRule` core needed. `applyInitialRuleToEntries` pre-fills + locks the Name entry. Submit "Save rule" ↔ "Update rule"; secondary "Clear" ↔ "Reset" (re-applies Initial). Edit mode does not auto-clear after save. `internal/ui/views/rules.go` — added 5th "Actions" column with Edit/Delete buttons; `RulesOptions` extended with `Remove` (defaults `core.RemoveRule`) and `OnRulesChanged`; `openEditRuleDialog` reuses `currentParentWindow` from accounts.go and shows form in `dialog.NewCustom`; `confirmDeleteRule` uses `dialog.ShowConfirm`. **Verified:** all 16 packages green under `nix run nixpkgs#go -- test -tags nofyne ./...`; `go vet -tags nofyne ./...` clean. UI completion now ~99.5%.

## Previous slice (kept for context)
Slice #39 landed: Edit Account form + per-row Edit/Delete buttons in Accounts view. New files: `internal/core/account_update.go` (`UpdateAccount(in)` alias-must-exist + blank-password-preserves + atomic via `config.WithWriteLock`); accounts.go per-row Edit/Delete via `dialog.NewCustom` + `dialog.ShowConfirm`; `currentParentWindow` helper introduced. UI completion was ~99%.
