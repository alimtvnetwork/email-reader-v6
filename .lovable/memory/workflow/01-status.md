...
Last updated: 2026-04-26 (UTC) — **Slice #37 landed: Add Account form extension (slice 1 of 2)** — Display Name field + Show/Hide password toggle. User correction: previous 5 slices (AC-DB AST guards) were off-track; user asked for UI form work, not backend tests. Honest UI audit confirmed all 7 nav destinations + both Add forms are wired in `internal/ui/app.go viewFor()`; UI is ~95% complete by surface, blocked only on user building the cgo binary on Windows to test. **Slice 1 changes:** `config.Account.DisplayName string` (omitempty for backward-compat), `core.AccountInput.DisplayName` plumbed through `buildAccount`, `views.AccountFormInput/Result.DisplayName` trimmed in `ValidateAccountForm`, new "Display name" Fyne entry row, new `newPasswordRevealButton` toggling `*widget.Entry.Password` in place + "Show/Hide" label swap, `clear()` resets reveal state to hidden, 2 new tests (`TestValidateAccountForm_DisplayNameTrimmed`, `TestValidateAccountForm_DisplayNameOptional`). All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -count=1 ./...`; `go vet -tags nofyne ./...` clean. **Slice 2 (queued for next `next`):** Provider preset dropdown (Gmail/Outlook/Yahoo/Custom auto-fills host/port/TLS) + Test Connection button (live IMAP probe with timeout + status banner before save).
