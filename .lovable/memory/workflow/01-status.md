...
| 198 | **README version surfacing (doc-only)** — Added `**Version:** 0.28.0` line at top of root `README.md` so users see the shipping CLI version without inspecting `cmd/email-read/main.go`. No code/spec impact; coverage unchanged. User also confirmed the 6 external blockers (incognito banner, bench infra, E2E harness, canvas harness #118e, race-gate CI, tools spec sign-off) are still pending a decision before any further sandbox-side progress is possible. | `README.md` | unchanged (51.6%) |
