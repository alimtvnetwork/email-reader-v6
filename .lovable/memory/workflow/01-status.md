...
Last updated: 2026-04-26 (UTC) — Slice #34 landed: AST guard test for AC-DB-47 (`Test_AST_MaintenanceOnly`). New `internal/store/ast_maintenance_only_test.go` walks the entire repo (skipping `.git`/`node_modules`/`.lovable`/`dist`/`build`), parses every production `.go` file (test files exempt), inspects every `*ast.BasicLit` of `STRING` kind, and rejects any literal whose trimmed body equals `VACUUM` / `ANALYZE` or starts with `pragma wal_checkpoint` (case-insensitive). The matcher is intentionally statement-shaped so incidental UI labels like `"Weekly VACUUM weekday"` don't trigger. Allowlist is a single entry: `internal/store/vacuum.go`. Function-length compliance via 5 small helpers (`scanRepoForMaintenanceSQL`, `inspectGoPath`, `skipUninterestingDir`, `candidateGoFile`, `scanFileForMaintenanceSQL`, `looksLikeMaintenanceSQL`, `repoRootForMaintenanceGuard`). All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -race -count=2 ./...`; `go vet -tags nofyne ./...` clean; fn-length linter 0 violations on the new file.
