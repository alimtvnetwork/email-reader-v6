...
Last updated: 2026-04-26 (UTC) — **Slice #39 landed: Edit Account form + per-row Edit/Delete buttons in Accounts view.** **New files:** `internal/core/account_update.go` (`UpdateAccount(in)` — alias-must-exist, blank password preserves existing PasswordB64, `mergeAccountUpdate` helper, `persistUpdateAccount` runs Load+Upsert+Save inside `config.WithWriteLock` for CF-A2 atomicity, publishes `AccountUpdated`); `internal/core/account_update_test.go` (3 tests: missing alias → ErrConfigAccountMissing, blank password keeps existing, non-blank replaces and base64-encodes); `internal/ui/views/account_form_edit_test.go` (2 tests for `AllowBlankPassword` validator branch). **Files edited:** `internal/ui/views/account_form.go` — added `AllowBlankPassword bool` to `AccountFormInput`; password-required check now skipped when set. `internal/ui/views/add_account_form.go` — added `Initial *config.Account` to `AddAccountFormOptions` (nil ⇒ Add mode, non-nil ⇒ Edit mode); `applyFormDefaults` routes Save to `core.UpdateAccount` in edit mode; `applyInitialToEntries` pre-fills + locks alias entry + swaps password placeholder to "leave blank to keep current"; submit button label flips "Save account" ↔ "Update account"; clear-button labelled "Reset" in edit mode and re-applies Initial; `editing` flag threaded through `newAccountSubmitButton` to set `AllowBlankPassword`. `internal/ui/views/accounts.go` — total rewrite to add per-row Edit/Delete buttons (6th column), `openEditAccountDialog` shows form in `dialog.NewCustom`, `confirmDeleteAccount` uses `dialog.ShowConfirm` then calls `core.RemoveAccount`, `currentParentWindow` discovers parent via `fyne.CurrentApp().Driver().AllWindows()`. `internal/ui/app.go` — passes `OnAccountsChanged` into `BuildAccounts` so successful Edit/Delete refreshes the sidebar account picker. **Verified:** all 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -count=1 ./...`; `go vet -tags nofyne ./...` clean. UI completion now ~99%.

## Previous slice (kept for context)
Slice #38 landed: Add Account form extension (slice 2 of 2) — Provider preset dropdown + Test Connection button. New files: `internal/ui/views/account_presets.go` (7 entries), `internal/mailclient/dial_plain.go` (`DialPlain` probe with timeout), `internal/core/test_connection.go` (`TestAccountConnection` orchestrator). Provider Select rendered first; Test connection button runs probe in goroutine with disabled-button + status-banner UX.
