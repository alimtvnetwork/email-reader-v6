...
Last updated: 2026-04-26 (UTC) — **Slice #41 landed: App boot smoke test.** **New file:** `internal/core/boot_smoke_test.go` (2 tests: `TestBootSmoke_FreshDataDir` walks the same data path `internal/ui/app.go::Run` does — `NewSettings` → `Settings.Get` → `Subscribe` → `ListAccounts` → `ListRules` — on a fresh data dir via `withIsolatedConfig`, asserts `Theme=ThemeDark`, `PollSeconds>0`, empty accounts/rules slices, and a 2s cold-start budget; `TestBootSmoke_RepeatedConstruction` rebuilds Settings 5× to catch subscriber-leak / global-state regressions). Tag-free so it runs under `-tags nofyne` (the project's standard verification path) — `internal/ui/app.go` itself is `!nofyne` so it cannot be exercised directly without a Fyne toolchain, but the bootstrap *data path* is what catches actual startup regressions (missing defaults, panics on absent config, perf cliffs). **Verified:** all 16 packages green under `nix run nixpkgs#go -- test -tags nofyne ./...`; `go vet -tags nofyne ./...` clean; `-race` clean on the new tests (1.03s).

## Previous slice (kept for context)
Slice #40 landed: Edit Rule form + per-row Edit/Delete buttons in Rules view. `Initial *config.Rule` on `AddRuleFormOptions` (Save reuses `core.AddRule`'s upsert-by-name semantics — no separate `UpdateRule` core needed); 5th "Actions" column in rules.go with `dialog.NewCustom` Edit + `dialog.ShowConfirm` Delete via the shared `currentParentWindow` helper. `rule_form_labels.go` extracted so labels are testable under nofyne.
