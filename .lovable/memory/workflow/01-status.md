...
Last updated: 2026-04-26 (UTC) — Slice #32 landed: chunked retention prune (`PruneBatchSize` knob now actually enforced). `internal/store/vacuum.go` introduces `DefaultPruneBatchSize=5000` + `PruneOpenedUrlsBeforeBatched(ctx, cutoff, batchSize) → (total, batches, err)` (rowid-IN-LIMIT subquery loop, terminates when a partial batch is observed, ctx-cancellable between batches); legacy `PruneOpenedUrlsBefore(ctx, cutoff)` now delegates with the default batch so all existing callers/tests stay green. UI runtime (`internal/ui/watch_runtime.go`) wraps the Pruner in a closure that re-reads `PruneBatchSize` from the live Settings snapshot on every tick via `pruneBatchFromSettings`. New tests in `internal/store/vacuum_batched_test.go`: under-batch single-shot, exact-multiple (terminator empty batch), many batches (ceil-N/B), zero-cutoff no-op, default-batch-size fallback. All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -race -count=2 ./...`. AC-DB-43 ("prune runs in batches of PruneBatchSize") now satisfied.
