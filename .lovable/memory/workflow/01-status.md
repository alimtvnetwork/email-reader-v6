...
Last updated: 2026-04-26 (UTC) — Slice #35 landed: AST guard test for AC-DB-50 (`Test_AST_DriverImportLimit`). New `internal/store/ast_driver_import_limit_test.go` walks the entire repo (skipping `.git`/`node_modules`/`.lovable`/`dist`/`build`), parses every production `.go` file (tests exempt) with `parser.ImportsOnly`, and rejects any file outside the `internal/store/` prefix that imports a known SQL driver. The driver allowlist `driverImportPaths` covers `modernc.org/sqlite`, `mattn/go-sqlite3`, `lib/pq`, `jackc/pgx/v5`(+stdlib), `go-sql-driver/mysql`, `microsoft/go-mssqldb`, and the bare `database/sql/driver` registrant — broad on purpose so a future contributor adding a new driver can't sneak past by switching backends. Function-length compliance via 4 small helpers (`scanRepoForDriverImports`, `inspectGoPathForImports`, `candidateGoFileForDriver`, `scanFileForDriverImports`) reusing `repoRootForMaintenanceGuard` + `skipUninterestingDir` from the maintenance-only guard. Today only `internal/store/store.go` imports `modernc.org/sqlite`; the guard passes clean. All 16 packages green under `nix run nixpkgs#go -- test -tags nofyne -race -count=2 ./...`; `go vet -tags nofyne ./...` clean; fn-length linter 0 violations on the new file.
