...
| 2 | `Result[T]` API   | Every `core.*` method returns `errtrace.Result[T]`                                                                 | 🟡 **In progress** — 2026-04-26: foundations landed. New `internal/errtrace/result.go` adds `Result[T]`, `Ok[T]`, `Err[T]`, `HasError`, `Value`, `Error`, `PropagateError`. New `internal/errtrace/codes.go` declares all `ErrXxx Code` constants from `06-error-registry.md`. New `WrapCode(err, code, op).WithContext(k, v)` builder is the canonical wrapper at the `core` boundary. **First migrated file:** `core.ExportCSV` now returns `errtrace.Result[string]` (caller `cli/rules_export.go:runExportCsv` updated). 9/9 errtrace tests pass. Remaining `core.*` files queued: `accounts.go`, `dashboard.go`, `diagnose.go`, `emails.go`, `read.go`, `rules.go`. |
