...
| 8 | Fn-length linter   | AC-PROJ-20: every Go fn ≤ 15 statements (`linter-scripts/check-fn-length.sh`)                                                                                                                                                | **2 violations** (down from 21 → … → 4 → 3 → 2). 2026-04-25 refactors: 11 fns refactored, latest `mailclient.SaveRaw` 17→≤15 (extracted `resolveSaveDir`/`buildRawFilename`). Remaining: `internal/store/store.go` (anon @ 254, 17); `internal/ui/views/dashboard.go:BuildDashboard(16)`. |
