...
| 4 | Design tokens     | `ColorWatchDot{Ok,Warn,Err}` + sidebar tokens + Fyne adapter + AST guards (`02-theme-implementation.md` §2/§6) | 🟢 **MVP+adapter+raw-log/badges/code groups+sizes/density/fonts done (2026-04-26)** — `internal/ui/theme/`: `tokens.go` (typed `ColorName` enum, **37 token constants**: §2.1 surfaces 7×, §2.2 brand 3×, §2.3 status 4×, §2.4 sidebar 6×, §2.5 watch dots 6×, §2.6 raw log 4×, §2.7 badges 3×, §2.8 code surfaces 4×), `palette_dark.go` + `palette_light.go` (key-parallel `map[ColorName]color.NRGBA` with **all 74 spec values verbatim** — both palettes parity-checked by AST-T5 + `Test_Palettes_Parity`), `theme.go` (public `Apply / Active / Color`; Apply is goroutine-safe via RWMutex; unknown token → ER-UI-21900 fallback to `ColorForeground`, dedup-logged once per name). **§3/§4 size scale (2026-04-26):** `sizes.go` adds typed `SizeName` enum + **15 size token constants** (7 typography `SizeText*` + 8 spacing `SizeSpacing0..7`), `sizeBase` resolver with all spec values verbatim (PageTitle 30 / SectionTitle 20 / CardTitle 16 / Body 14 / Caption 12 / Code 13 / Button 14; Spacing 0/4/8/12/16/24/32/48), `Size(SizeName) float32` accessor with no-panic unknown→0 fallback (logged via shared dedup channel as `ER-UI-21900` namespaced `size:…`). **§8 density resolver:** `Density` enum (`DensityComfortable` default / `DensityCompact`); `SetDensity / ActiveDensity` under the same RWMutex as the theme mode; Compact applies the spec ×0.875 multiplier with banker's rounding via `math.Round`. **§3 fonts:** `fonts.go` declares `embed.FS` over `all:fonts` and exposes `TextFont() / TextMonospaceFont() fyne.Resource`; assets live at `fonts/Inter-Variable.ttf` + `fonts/JetBrainsMono-Variable.ttf` (OFL). When the .ttf files are absent (fresh checkout, pre-asset CI) the embed FS is empty and both accessors return nil — `AppTheme.Font(...)` falls back to `fynetheme.DefaultTheme().Font(s)` so the UI never breaks; once committed, the same code path returns the embedded resource with zero re-wiring. **§2.6 raw-log tokens:** `ColorRawLogHeartbeat / ColorRawLogNewMail / ColorRawLogError / ColorRawLogTimestamp` — closes OI-1 second half. **§2.7 badge tokens:** `ColorRuleMatchBadge / ColorBadgeNeutralBg / ColorBadgeNeutralFg` — power Badge widget variants. **§2.8 code-surface tokens:** `ColorCodeBg / ColorCodeBorder / ColorCodeLineHighlight / ColorCodeSelection`. **Fyne adapter:** `fyne_theme.go` (build-tagged `!nofyne`) implements `fyne.Theme` with the §2 color routing table (11 entries) AND the new **§3/§4 size routing table** (`fyneSizeRoute`: SizeNameText→TextBody, SizeNameHeadingText→TextPageTitle, SizeNameSubHeadingText→TextSectionTitle, SizeNameCaptionText→TextCaption, SizeNamePadding→Spacing2, SizeNameInnerPadding→Spacing3, SizeNameInlineIcon→TextCaption); unmapped names fall through to Fyne defaults. `Font(s)` routes monospace styles to `TextMonospaceFont()` (JetBrains Mono) and everything else to `TextFont()` (Inter Variable handles weights via the variable axis), with nil-fallback to defaults. `fyne_apply.go` adds `ApplyToFyne(mode)`. **Bootstrap wired:** `internal/ui/app.go::Run` reads `core.Settings.Get().Theme` and calls `theme.ApplyToFyne` BEFORE `BuildShell`. **AST guards:** AST-T1, AST-T2, AST-T4, AST-T5 — auto-validate new groups land in both palettes. **New error code:** `ER-UI-21900 ErrUiThemeUnknownToken`. **Tests (16 cases — 5 added today):** Palettes_Parity, Color_ResolvesPerMode, Color_KnownTokens, Color_RawLogBadgeCodeTokens, Color_UnknownFallback, Apply_RejectsInvalid, Apply_ConcurrentSafe, **Size_ComfortableScale** (NEW — locks all 15 spec px values), **Size_CompactScale** (NEW — locks ×0.875 rounded values for body/page/section/card/caption + 4 spacing spot checks), **Size_DensityToggleNoStateLeak** (NEW — flips back-and-forth without re-Apply), **Size_UnknownReturnsZero** (NEW — no-panic contract), **AllSizeNames_CoversBase** (NEW — drift guard), **Fonts_FallbackWhenAbsent** (NEW — embed-or-nil contract; both accessors agree on present/absent state), plus the 4 AST guards. 14 packages green under `-tags nofyne`; fn-length linter still **0/0** (66 files). **Settings live theme-switch consumer:** `internal/ui/app.go::startThemeLiveConsumer` + `internal/ui/theme_live_test.go`. **Outstanding:** (a) drop the actual Inter-Variable.ttf + JetBrainsMono-Variable.ttf binary assets under `internal/ui/theme/fonts/` (wiring is done; only the OFL payloads remain); (b) selection alpha-blend (currently solid Primary); (c) Settings UI surface for Density toggle (resolver wired + tested, just no UI control yet); (d) tabular-numeric `tnum` OpenType feature flag plumbing (§3.2). |
