...
| 8 | Fn-length linter   | AC-PROJ-20: every Go fn ≤ 15 statements (`linter-scripts/check-fn-length.sh`)                                                                                                                                                | **7 violations** (down from 21 → 12 → 11 → 9 → 8 → 7). 2026-04-25 refactors: `BuildAddAccountForm` 38→≤15, `BuildAddRuleForm` 33→≤15, `runAdd` 30→≤15, `watcher.Run` 21→≤15, `watcher.pollOnce` 26→≤15, `core.ReadEmail` 28→≤15 (extracted `loadEmailDetail`/`ensureSeededRule`/`buildEngineAndLauncher`/`rowToMessage`/`evaluateMatches`/`openMatches`/`openOneMatch`), `core.Diagnose` 23→≤15 (extracted `resolveAccount`/`probeConnect`/`listFolders`/`fetchInbox`/`fetchHeaders`/`scanFolders`/`runFolderScanAndSummarize`). Remaining: `internal/cli/cli.go:runWatch(18)`; `internal/core/accounts.go:AddAccount(18)`; `internal/exporter/exporter.go:ExportCSV(18)`; `internal/mailclient/mailclient.go:SaveRaw(17)`; `internal/store/store.go` (anon @ 254, 17); `internal/ui/views/dashboard.go:BuildDashboard(16)`; `internal/ui/views/emails.go:BuildEmails(21)`. |
