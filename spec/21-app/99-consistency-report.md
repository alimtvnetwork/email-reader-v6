...
| 8 | Fn-length linter   | AC-PROJ-20: every Go fn ≤ 15 statements (`linter-scripts/check-fn-length.sh`)                                                                                                                                                | **1 violation** (down from 21 → … → 3 → 2 → 1). 2026-04-25 refactors: 12 fns refactored, latest `store.ListEmails` 17→≤15 (extracted `buildListEmailsQuery`/`scanEmailRows`). Remaining: `internal/ui/views/dashboard.go:BuildDashboard(16)`. |
