...
| 2 | `Result[T]` API   | Every `core.*` method returns `errtrace.Result[T]`                                                                 | 🟡 **In progress (3/7)** — 2026-04-26: foundations + 3 files migrated. **Foundations:** `internal/errtrace/result.go` (Result[T], Ok/Err, HasError/Value/Error/PropagateError, Coded carrier, WrapCode, WithContext); `internal/errtrace/codes.go` (all ErrXxx Code constants from 06-error-registry.md); `Frames()` extended to walk both *Traced and *Coded chains. **Migrated:** `core.ExportCSV` → `Result[string]`; `core.AddAccount/ListAccounts/GetAccount/RemoveAccount` → `Result[*AddAccountResult]/[]config.Account/config.Account/struct{}`; `core.LoadDashboardStats` → `Result[DashboardStats]` (uses ErrConfigOpen + ErrDbQueryEmail with WithContext("scope", "total"|"alias")). **Callers updated:** `cli/rules_export.go`, `cli/cli.go` (runAddQuick, runAdd, runList, runRemove), `ui/app.go` (LoadAliases), `ui/views/accounts.go` (AccountsOptions.List), `ui/views/add_account_form.go` (AddAccountFormOptions.Save), `ui/views/dashboard.go` (LoadStatsFunc + makeDashboardRefresh closure now consumes Result envelope). **Drive-by fixes:** corrected pre-existing build breaks in `core/read.go` (loadEmailDetail returned config.Config-by-value vs *Config; ensureSeededRule passed *cfg to Save instead of cfg), `core/diagnose.go` (unused cfg var), `watcher/watcher.go` (persistMessage / evaluateRules / processMessage / fetchAndCheckEmpty / processBatch / advanceCursor: []mailclient.Message → []*mailclient.Message to match SaveRaw/EvaluateWithTrace pointer signatures), `cli/cli.go:497` (*cfg deref). All 9 non-Fyne packages compile + tests green: errtrace, core, cli, watcher, exporter, store, config, rules, mailclient. **Remaining `core.*` files queued:** `diagnose.go`, `emails.go`, `read.go`, `rules.go`. |
