...
| 2 | `Result[T]` API   | Every `core.*` method returns `errtrace.Result[T]`                                                                 | 🟡 **In progress (6/7)** — 2026-04-26: foundations + 6 files migrated. **Migrated:** `core.ExportCSV` → `Result[string]`; `core.AddAccount/ListAccounts/GetAccount/RemoveAccount`; `core.LoadDashboardStats` → `Result[DashboardStats]`; `core.Diagnose` → `Result[struct{}]`; `core.ListEmails`/`GetEmail`/`CountEmails`; `core.AddRule` → `Result[*AddRuleResult]`, `core.ListRules` → `Result[[]config.Rule]`, `core.GetRule` → `Result[config.Rule]`, `core.SetRuleEnabled`/`RemoveRule` → `Result[struct{}]` (using ErrCoreInvalidArgument, ErrRulePatternInvalid w/ WithContext("field",…), ErrConfigOpen/Encode, ErrRuleNotFound w/ WithContext("name",…)). **Callers updated:** `cli/rules_export.go` (newRulesAddCmd, runRulesList, runRulesToggle now PropagateError); `ui/views/rules.go` (RulesOptions.List → Result[[]config.Rule], Toggle → Result[struct{}]; reload/ruleRow unwrap via HasError/Error/Value); `ui/views/add_rule_form.go` (AddRuleFormOptions.Save → Result[*AddRuleResult]; submit handler unwraps). **Tests updated:** `core/rules_test.go`. All packages compile + tests green (errtrace, core, cli, watcher, exporter, store, config, rules, mailclient, browser, imapdef, ui, ui/views). **Remaining `core.*` files queued:** `read.go`. |
