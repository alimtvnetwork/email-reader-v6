...
| 3 | Settings backend  | `core.Settings` service with atomic `Save` (tmp + fsync + rename) and `Subscribe`                                  | 🟢 **MVP done (2026-04-26)** — `internal/core/settings.go` + `settings_types.go` + `settings_validate.go` + `settings_extension.go`. Public API `NewSettings(clock) → Result[*Settings]` with `Get / Save / ResetToDefaults / DetectChrome / Subscribe`. **Atomic write** via single tmp+rename combining typed config + new `settings` extension block (preserves Accounts/Rules byte-for-byte; verified by `TestSettings_SavePreservesAccountsAndRules`). **Validation:** all §6 codes wired (`ER-SET-21770..21777` added to `errtrace/codes.go`). **Pub/sub:** buffered cap-4 channels with non-blocking publish + cancel-closes-channel semantics. **Chrome detection:** §7 probe order (Config → Env `EMAIL_READ_CHROME` → OS defaults → PATH → NotFound). **Tests:** 11 cases in `settings_test.go` covering defaults, round-trip, normalize/dedupe, all 6 validation codes, ChromePath stat, Reset, Subscribe delivery+cancel, DetectChrome. Race-clean (`go test -race`). All 13 packages green. **Schema deviation:** keeps legacy camelCase JSON keys (`pollSeconds`, `chromePath`) for backward compat; PascalCase migration deferred to Delta #1. |
