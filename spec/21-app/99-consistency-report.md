...
| 8 | Fn-length linter   | AC-PROJ-20: every Go fn ≤ 15 statements (`linter-scripts/check-fn-length.sh`)                                                                                                                                                | **5 violations** (down from 21 → 12 → 11 → 9 → 8 → 7 → 6 → 5). 2026-04-25 refactors: `BuildAddAccountForm` 38→≤15, `BuildAddRuleForm` 33→≤15, `runAdd` 30→≤15, `watcher.Run` 21→≤15, `watcher.pollOnce` 26→≤15, `core.ReadEmail` 28→≤15, `core.Diagnose` 23→≤15, `BuildEmails` 21→≤15, `cli.runWatch` 18→≤15 (extracted `resolveWatchAccount`/`seedDefaultRuleIfMissing`/`buildWatchEngineAndLauncher`/`newWatchContext`). Remaining: `internal/core/accounts.go:AddAccount(18)`; `internal/exporter/exporter.go:ExportCSV(18)`; `internal/mailclient/mailclient.go:SaveRaw(17)`; `internal/store/store.go` (anon @ 254, 17); `internal/ui/views/dashboard.go:BuildDashboard(16)`. |
