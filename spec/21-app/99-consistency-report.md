...
| 6 | Doctor + Tools UI  | `email-read doctor` rune-dump + Tools card                                                                         | 🟢 **MVP done (2026-04-26)** — `core.Doctor(target) → Result[[]DoctorReport]` (`internal/core/doctor.go`) is the structured backend powering both surfaces; CLI `doctor` refactored to call it via `runDoctor` + `printDoctorReport`. New `config.DecodeRawPassword` exposes pre-sanitization bytes. **Tools UI:** `internal/ui/views/tools_doctor.go::BuildDoctorTab` renders one Card per account (⚠ flag when Hidden=true) with monospace rune-dump tables (sanitized + raw). `internal/ui/views/tools_diagnose.go::BuildDiagnoseTab` calls `core.Diagnose` with a streaming event renderer that mirrors the CLI's step-numbered output. **OpenUrl sub-tool MVP (2026-04-26):** `core.Tools` slim slice landed at `internal/core/tools.go` + `tools_redact.go` per `02-features/06-tools/01-backend.md` §2.4 + `00-overview.md` §5: `NewTools(browser, store, cfg) → Result[*Tools]` with cfg validation (schemes ⊂ {http,https}, length ∈ [256,65536], dedup ∈ [0,3600]); `OpenUrl(ctx, spec) → Result[OpenUrlReport]` runs the full §5.1 validation pipeline (empty / too-long / malformed / scheme-forbidden / no-host / loopback / private-IP — first failure short-circuits with `ER-TLS-21760..21765`), §5.2 redaction (strips userinfo, masks `password|pwd|secret|token|code|otp` query keys via `redactUrl`), per-key in-memory dedup with 60 s window (returns `Deduped=true` without launching), then incognito launch via `browser.Launcher.Open` and best-effort audit insert. New error block `ER-TLS-21750..21767` added to `internal/errtrace/codes.go`. **Tools UI:** `internal/ui/views/tools_openurl.go::BuildOpenUrlTab` replaces the placeholder — URL entry + Launch button + status line; constructs a fresh `core.Tools` per launch so live `cfg.Browser` edits take effect without restart. **Tests (5 cases):** `TestOpenUrl_HappyPath` (1 launch, IsIncognito=true, BrowserBinary set), `TestOpenUrl_ValidationFailures` (8 sub-cases × 8 codes; asserts no browser launch), `TestOpenUrl_DedupInMemory` (2nd call deduped, 1 total launch), `TestOpenUrl_RedactsUserinfoAndSecrets` (canonical strips `user:pw` + masks `token=`, original preserved), `TestNewTools_ConfigValidation` (empty schemes, tiny length). 14 packages green; fn-length linter still 0/0. **Outstanding:** Read / Export CSV tabs still placeholders; `Diagnose` cache + `RecentOpenedUrls` accessor + `AccountEvent` cache invalidation deferred to next slice. |
