...
| 2 | `Result[T]` API   | Every `core.*` method returns `errtrace.Result[T]`                                                                 | 🟡 **In progress (5/7)** — 2026-04-26: foundations + 5 files migrated. **Foundations:** `internal/errtrace/result.go` (Result[T], Ok/Err, HasError/Value/Error/PropagateError, Coded carrier, WrapCode, WithContext); `internal/errtrace/codes.go` (all ErrXxx Code constants from 06-error-registry.md); `Frames()` extended to walk both *Traced and *Coded chains. **Migrated:** `core.ExportCSV` → `Result[string]`; `core.AddAccount/ListAccounts/GetAccount/RemoveAccount` → Result variants; `core.LoadDashboardStats` → `Result[DashboardStats]`; `core.Diagnose` → `Result[struct{}]`; `core.ListEmails` → `Result[[]EmailSummary]`, `core.GetEmail` → `Result[*EmailDetail]`, `core.CountEmails` → `Result[int]` (all use ErrDbOpen + ErrDbQueryEmail with WithContext("alias",…) / WithContext("uid",…)). **Callers updated:** `cli/rules_export.go`, `cli/cli.go` (runAddQuick, runAdd, runList, runRemove, runDiagnose), `ui/app.go`, `ui/views/accounts.go`, `ui/views/add_account_form.go`, `ui/views/dashboard.go`, `ui/views/emails.go` (EmailsOptions.List/Get seam types now return Result envelope; loadEmailRows + makeEmailSelectHandler unwrap with HasError/Value/Error). **Internal callers updated:** `core/dashboard.go` LoadDashboardStats now consumes the new Result-flavored CountEmails. **Tests updated:** `core/diagnose_test.go`. **Drive-by fixes:** `core/read.go`, `watcher/watcher.go` (slice-of-pointer signatures), `cli/cli.go:497`. All 12 non-Fyne + Fyne packages compile + tests green: errtrace, core, cli, watcher, exporter, store, config, rules, mailclient, browser, imapdef, ui, ui/views. **Remaining `core.*` files queued:** `read.go`, `rules.go`. |
