...
| 3 | Settings backend  | `core.Settings` service with atomic `Save` (tmp + fsync + rename) and `Subscribe`                                  | 🟢 **MVP done (2026-04-26)** — `internal/core/settings.go` + `settings_types.go` + `settings_validate.go` + `settings_extension.go`. Public API `NewSettings(clock) → Result[*Settings]` with `Get / Save / ResetToDefaults / DetectChrome / Subscribe`. **Live consumers wired:** **CF-W1** poll cadence (watcher.Options.PollSecondsCh, applied on next tick); **CF-T1 (2026-04-26)** browser launcher live ChromePath via `browser.Launcher.Reload(cfg)` — replaced legacy `sync.Once` with a mutex-protected `resolved` flag so Reload re-arms the resolver, snapshot reads (Path/IncognitoArg) hold the lock to be race-clean (verified with `-race`); fan-out from a single Settings subscription via `cli.startReloadBridges` + `forwardSettingsEvents` (combined into one helper to keep `runWatch` ≤15 statements). Mid-launch URL opens are never interrupted (they completed before Reload arrived). **CF-D1 (2026-04-26)** Dashboard "Auto-start watcher" indicator (`internal/ui/views/dashboard.go::newAutoStartIndicator`) shows ● ON / ○ off and updates on every SettingsEvent via a background `forwardAutoStartEvents` goroutine; on Settings setup failure it shows neutral "Auto-start watcher: unknown" — never blocks the UI. **Settings live theme-switch (Delta #4 (e), 2026-04-26)** also wired via `internal/ui/app.go::startThemeLiveConsumer` + `forwardThemeEvents` (dedupes unchanged Theme). **Tests:** 11 settings cases + 5 poll-reload cases + 2 `Test_Reload_*` browser cases (race-clean: rearm + concurrent) + 2 theme-live cases. 14 packages green; fn-length linter still 0/0. **Schema deviation:** keeps legacy camelCase JSON keys; PascalCase migration deferred to Delta #1. **Outstanding (CF-W3, CF-W2):** Watch frontend cadence label (CF-W3) blocked on the Watch view existing — `internal/ui/views/` has no `watch.go` yet (current "watch" surface is the CLI loop); will land with the Watch view per `02-features/05-watch/02-frontend.md`. CF-W2 (AutoStartWatch read once at process start) is a **constraint test** rather than wiring — to be added when an auto-start handler exists. |
