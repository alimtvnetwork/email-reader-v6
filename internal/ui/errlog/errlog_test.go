// errlog_test.go — unit tests for the in-memory error-log ring buffer
// (slice 3.1/3.2 of the error-trace upgrade — see .lovable/plan.md).
package errlog
...
func TestStore_AppendAndSnapshot(t *testing.T) {
	s := NewStore(3)
	s.Append(Entry{Component: "a", Summary: "1"})
	s.Append(Entry{Component: "b", Summary: "2"})
	got := s.Snapshot()
	if len(got) != 2 {
...
func TestStore_RingEvictsOldest(t *testing.T) {
	s := NewStore(2)
	for i := 0; i < 5; i++ {
		s.Append(Entry{Summary: time.Now().String()})
	}
	got := s.Snapshot()
	if len(got) != 2 {
...
func TestStore_SubscribeReceivesAppends(t *testing.T) {
	s := NewStore(10)
	ch := s.Subscribe()
	s.Append(Entry{Summary: "hello"})
	select {
	case e := <-ch:
		if e.Summary != "hello" {
...
func TestStore_SlowSubscriberDoesNotBlockProducer(t *testing.T) {
	s := NewStore(100)
	ch := s.Subscribe() // buffer 16, never drained
	done := make(chan struct{})
	go func() {
		for i := 0; i < 200; i++ {
			s.Append(Entry{Summary: "x"})
		}
		close(done)
	}()
	select {
...
func TestStore_ConcurrentAppends(t *testing.T) {
	...
	for g := 0; g < 10; g++ {
		...
		go func() {
			defer wg.Done()
			for i := 0; i < 50; i++ {
				s.Append(Entry{Summary: "concurrent"})
			}
		}()
	}
	wg.Wait()
